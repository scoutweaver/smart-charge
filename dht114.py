...
class DHT114Result:
	'DHT114 sensor result returned by DHT114.read() method'
	...
	def is_valid(self):
		return self.error_code == DHT114Result.ERR_NO_ERROR

class DHT114:
	'DHT114 sensor reader class for Raspberry'
	...
	def read(self):
		...
		if len(pull_up_lengths) != 40:
			return DHT114Result(DHT114Result.ERR_MISSING_DATA, 0, 0)
		...
		if the_bytes[4] != checksum:
			return DHT114Result(DHT114Result.ERR_CRC, 0, 0)
			
		# ok, we have valid data, return it
		return DHT114Result(DHT114Result.ERR_NO_ERROR, the_bytes[2], the_bytes[0])
...
Temp_sensor = 14    #assigning the pin for the temperature sensor
instance = DHT114(pin = Temp_sensor)     #set up the function above

def read_temp_humid(cur, numb, temp, humid):
    #temp = 0
    #humid = 0
    for x in range(0,10):     #checks 10 times per call since the sensors dont always respond
        result = DHT114(numb).read()  #actually read the values
...
x = read_temp_humid(10, 7, 0, 0)
print(x)

def readwrite(cur,numb):   #reads all sensor and voltage values and writes them to the db
